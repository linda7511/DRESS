...
if __name__ == "__main__":
    ...
    """
        For FFSD dataset
    """
    print(f"processing FFSD data...")
    data = pd.read_csv(os.path.join(DATADIR, 'FFSD.csv'))
    data = featmap_gen(data.reset_index(drop=True))
    data.replace(np.nan, 0, inplace=True)
    data.to_csv(os.path.join(DATADIR, 'FFSDneofull.csv'), index=None)
    data = pd.read_csv(os.path.join(DATADIR, 'FFSDneofull.csv'))
    ...
    g.ndata['feat'] = torch.from_numpy(
        feat_data.to_numpy()).to(torch.float32)
    dgl.data.utils.save_graphs(DATADIR + "graph-FFSD.bin", [g])

    # generate neighbor riskstat features
    for file_name in ['FFSD', 'yelp', 'amazon']:
        print(
